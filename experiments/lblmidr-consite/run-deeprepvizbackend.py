...
ID_col = 'subjectID'
label = 'lbl_lesion'
DATASET_SEARCH_PATH = "dataset/toybrains_*{}/toybrains_*{}.csv"
LOGDIR_SEARCH_PATH = "log/toybrains*{}*/trial*/deeprepvizlog/"
...
if __name__ == "__main__":
    ...
    DATASET_UNIQUE_IDS = sorted([os.path.basename(logdir).split('_')[3] for logdir in glob("log/toybrains_n*") \
    ...
    def run_backend(dataset_unique_ID, label=label, ID_col=ID_col, 
        ...
        attributes_table = glob(DATASET_SEARCH_PATH.format(dataset_unique_ID, dataset_unique_ID))
        ...
        logdirs = sorted([logdir for logdir in glob(LOGDIR_SEARCH_PATH.format(dataset_unique_ID)) if 'debug' not in logdir])
        if len(logdirs)==0:
    ...
    joblib.Parallel(n_jobs=-1)(joblib.delayed(
